...
for i = locs(end)-1:-1:1
    ...
    x0 = [del_brake(i+1), beta_brake(i+1),ax_brake(i+1),br_vel(i+1)/whl_radius,br_vel(i+1)/whl_radius,br_vel(i+1)/whl_radius,br_vel(i+1)/whl_radius,br_vel(i+1)];
    Aeq = [];
    beq = [];
    lb = [min_steering, beta_min,-40,0,0,0,0,0];
    ub = [max_steering, beta_max,0,29/whl_radius,29/whl_radius,29/whl_radius,29/whl_radius,29]; % Change Upper Bound of Deceleration to Zero
