...
optOut = sym(-x(1)); % Maximise velocity through corner

gradLSP = jacobian(optOut,x).';

hessLSP = jacobian(gradLSP,x);

currdir = [pwd filesep 'jacobians' filesep]; % You might need to use currdir = pwd 
filename = [currdir,'objectiveFunction.m'];
matlabFunction(optOut,gradLSP,'file',filename,'vars',{x});

filename = [currdir,'objectiveHessian.m'];
matlabFunction(hessLSP,'file',filename,'vars',{x});
